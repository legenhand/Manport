...
if($cek_has_nilai_deskripsi > 0) {
    ...
    $ketemu = false;
    $nilai_anyar = array();
    if(strpos($kelasJurusan, 'X TKJ') !== false){
        $urut_mapel = $urut_mapel_tkj_x;
    }elseif(strpos($kelasJurusan, 'XI TKJ') !== false){
        $urut_mapel = $urut_mapel_tkj;
    }elseif (strpos($kelasJurusan, 'XI BDP') !== false){
        $urut_mapel = $urut_mapel_bdp;
    }elseif (strpos($kelasJurusan, 'XI OTKP') !== false){
        $urut_mapel = $urut_mapel_otkp;
    }elseif(strpos($kelasJurusan, 'X BDP') !== false){
...
$tandaTangan = '
	<table border="0" cellspacing="" cellpadding="8" style="font-size: 10px">
		<tr>
